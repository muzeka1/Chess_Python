    ...
    def find_possible_move(self):
        ...
        if self.figure == "♙":
            if self.color == "white":
                ...
                if x+1 <= 7 and y-1 >= 0 and board[x+1][y-1] != "" and cells_board[x+1][y-1].cget("fg") != self.color:
                    self.possible_moves.append([x+1,y-1])
                if x+1 <= 7 and y+1 <= 7 and board[x+1][y+1] != "" and cells_board[x+1][y+1].cget("fg") != self.color:
                    self.possible_moves.append([x+1,y+1])
            if self.color == "black":
                ...
                if x-1 >= 0 and board[x-1][y] == "":
                    self.possible_moves.append([x-1, y])
                if x-1 >= 0 and y-1 >= 0 and board[x-1][y-1] != "" and cells_board[x-1][y-1].cget("fg") != self.color:
                    self.possible_moves.append([x-1,y-1])
                if x-1 >= 0 and y+1 <= 7 and board[x-1][y+1] != "" and cells_board[x-1][y+1].cget("fg") != self.color:
                    self.possible_moves.append([x-1,y+1])
...
class Main():
    ...
    def near_king_moves_func(self, enemy_color): #В процессе разработки. Функция для подсчета всех ходов всех фигур. Пока не продумал её до конца
        for i in range(0, 8):
            for j in range(0, 8):
                if board[i][j] != "" and board[i][j] != "♔" and cells_board[i][j].cget("fg") == enemy_color:
                    #print(i, j)
                    object_class_movement_for_king = Movement([i, j], board[i][j], enemy_color, self.possible_moves_main, True)
                    #Movement.find_possible_move(self, [i, j], self.board[i][j], enemy_color, True)
                    
    def move(self, cell_name):
        current_btn = cells_board[cell_name[0]][cell_name[1]]
        ...
        if not self.isPressed and current_btn.cget("text") != "" and current_btn.cget("fg") == self.turn_color:
            ...
            object_class_movement = Movement(cell_name, self.pressed_btn_text, self.turn_color, self.possible_moves_main, False)
